...
public class TestR1_Persons {
	...
	public void testAddMultiplePersons() {
		Set<String> codes = generateAndAddPerson(100, v);
	...
	public void testPersonsInIntervals() {
		generateAndAddPerson(10000, v);
		v.setAgeIntervals(30, 50, 60, 70);

		Collection<String> people = v.getInInterval("[70,+)");

		assertNotNull(people);
		assertEquals("Missing people in interval", 0.30, people.size() / (double) v.countPeople(), 0.02);

		assertEquals("Missing people in interval", 0.10, v.getInInterval("[60,70)").size() / (double) v.countPeople(),
				0.01);
	}
}
