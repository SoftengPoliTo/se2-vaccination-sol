...
public class TestUtils {
	...
	/**
	 * Generates fake people information and and return the relative information as
	 * a set.
	 * 
	 * 
	...
	/**
	 * Generates fake people information and and optionally adds them to the vaccine
	 * system.
	 * 
	...
	public static Set<String> generateAndAddPerson(int n, Vaccines system) {
		...
		while (cfs.size() < n) {
			...
			String cf = toCode(last) + toCode(first) + (year % 100) + place + cin1 + cin2; // generate a fake ssn
																							// (codice fiscale)
			...
			if (cfs.add(cf)) {
				people.add(person);
				if (system != null)
					system.addPerson(first, last, cf, year);
			}
		}
		return people;
	}
