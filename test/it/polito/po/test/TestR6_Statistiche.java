...
import static it.polito.po.test.TestUtils.generateAndAddPerson;
...
public class TestR6_Statistiche {
	...
	/*
	 * @Test public void testPropAllocatedAge() throws VaccineException {
	 * Map<String,Double> allInAge = v.propAllocatedAge(); assertNotNull(allInAge);
	 * assertEquals(5,allInAge.size()); double over70 = allInAge.get("[70,+)"); //
	 * due to ambiguity between sample tests and requirements // both values are
	 * accepted by the test
	 * assertTrue("Wrong proportion: expected 16% or 53% but was " +
	 * Math.round(over70*100) + "%", Math.abs(over70-0.16) < 0.01 || Math.abs(over70
	 * - 0.53) < 0.01); }
	 */
