...
public class TestR5_Pianificazione {
	...
	public void testAllocateWeek() {
		...
		Map<Integer, Long> freqs = wp.stream().flatMap(dm -> dm.values().stream().flatMap(List::stream))
				.collect(Collectors.groupingBy(v::getAge, Collectors.counting()));

		long allocated = freqs.values().stream().mapToLong(i -> i).sum();
		assertEquals("Unallocated people", 6800, allocated);

		double propOver70 = freqs.entrySet().stream().filter(e -> e.getKey() >= 70).mapToLong(e -> e.getValue()).sum()
				/ (double) allocated;
		assertEquals("Wrong proportion of over 70", 0.48, propOver70, 0.01);

		double propUnder30 = freqs.entrySet().stream().filter(e -> e.getKey() < 30).mapToLong(e -> e.getValue()).sum()
				/ (double) allocated;
